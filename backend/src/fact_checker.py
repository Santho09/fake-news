...
class FactChecker:
    def __init__(self, google_api_key=None):
        ...
        try:
            ...
            # Keep console logs ASCII-only to avoid Windows encoding issues.
        ...
        if self.use_google_api:
            print("[OK] Google Fact Check API enabled")
        else:
            print("[INFO] Google Fact Check API not configured (optional)")
    ...
    def check_google_fact_check(self, text: str) -> List[Dict]:
        ...
        if not self.use_google_api:
            print("[INFO] Google API not enabled (no API key)")
            return []
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"[WARN] Google API returned status {response.status_code}")
            ...
            if 'claims' not in data:
                print("   [INFO] No claims found in Google API response")
                return []
            
            print(f"   [OK] Found {len(data.get('claims', []))} claims")
            ...
            print("[WARN] Google Fact Check API timeout")
            return []
        except Exception as e:
            print(f"[WARN] Google Fact Check API error: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_verdict(self, ml_prediction: Optional[bool], ml_confidence: float, fact_check_result: Dict) -> Dict:
        """
        Combine ML prediction with fact-checking results.
        ml_prediction:
          - True  => ML says FAKE
          - False => ML says REAL
          - None  => ML says UNCERTAIN
        ...
        confidence_adjustment = int(fact_check_result.get('confidence_adjustment', 0) or 0)
        scam_issues = fact_check_result.get('scam_issues', []) or []
        factual_issues = fact_check_result.get('factual_issues', []) or []
        google_fact_checks = fact_check_result.get('google_fact_checks', []) or []

        critical_scam_count = sum(1 for s in scam_issues if s.get('severity') == 'critical')
        high_factual_count = sum(1 for f in factual_issues if f.get('severity') == 'high')
        medium_issue_count = (
            sum(1 for s in scam_issues if s.get('severity') == 'medium')
            + sum(1 for f in factual_issues if f.get('severity') == 'medium')
        )
        google_false_hits = 0
        for fc in google_fact_checks:
            rating = str(fc.get('rating', '')).lower()
            if any(k in rating for k in ['false', 'misleading', 'incorrect', 'inaccurate', 'disputed']):
                google_false_hits += 1

        # Strong override only for high-confidence fact-check red flags.
        strong_fact_red_flag = (
            critical_scam_count > 0
            or high_factual_count > 0
            or google_false_hits > 0
            or confidence_adjustment >= 35
        )

        if strong_fact_red_flag:
            return {
                'verdict': 'FAKE NEWS',
                'adjusted_confidence': 95,
                'color': 'red',
                'reason': '🚨 Strong fact-check red flags detected (critical claim-risk patterns found).'
            }

        if ml_prediction is None:
            if len(warnings) > 0 or medium_issue_count > 0:
                return {
                    'verdict': 'UNCERTAIN',
                    'adjusted_confidence': 50,
                    'color': 'orange',
                    'reason': 'ML was uncertain and fact-check found caution signals. Manual verification recommended.'
                }
            return {
                'verdict': 'UNCERTAIN',
                'adjusted_confidence': 50,
                'color': 'orange',
                'reason': 'ML confidence is low. The article needs manual verification.'
            }

        # Mild issues should not force FAKE when ML says REAL.
        # Instead, keep verdict and lower confidence to communicate uncertainty.
        if (len(warnings) > 0 or medium_issue_count > 0) and not ml_prediction:
            adjusted = max(45.0, float(ml_confidence) - min(20.0, confidence_adjustment * 0.5))
            return {
                'verdict': 'REAL NEWS',
                'adjusted_confidence': round(adjusted, 2),
                'color': 'green',
                'reason': 'ML leans REAL, but fact-check found some caution signals. Verify with trusted sources.'
            }
        ...
        if ml_prediction:
            return {
                'verdict': 'FAKE NEWS',
                'adjusted_confidence': round(float(ml_confidence), 2),
                'color': 'red',
